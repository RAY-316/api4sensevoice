...
reg_spks_files = ["speaker/speaker1_a_cn_16k.wav"]
...
def speaker_verify(audio, sv_thr):
    ...
    for k, v in reg_spks.items():
        ...
        if res_sv["score"] >= sv_thr:
            hit = True
        logger.info(
            f"[speaker_verify] audio_len: {len(audio)}; sv_thr: {sv_thr}; hit: {hit}; {k}: {res_sv}"
        )
    return hit, k
...
async def custom_exception_handler(request: Request, exc: Exception):
    ...
    return JSONResponse(
        ...
        content=TranscriptionResponse(
            code=status_code, msg=message, data=data
        ).model_dump(),
    )
...
async def websocket_endpoint(websocket: WebSocket):
    try:
        logger.info(f"[DEBUG] websocket.scope: {websocket.scope}")
        ...
        while True:
            ...
            audio_buffer = np.append(
                audio_buffer,
                np.frombuffer(
                    buffer[: len(buffer) - (len(buffer) % 2)], dtype=np.int16
                ).astype(np.float32)
                / 32767.0,
            )
                        ...
                        if not hit:
                            ...
                            if hit:
                                response = TranscriptionResponse(
                                    code=2, info="detect speaker", data=speaker
                                )
                                await websocket.send_json(response.model_dump())
                    else:
                        response = TranscriptionResponse(
                            code=2, info="detect speech", data=""
                        )
                        await websocket.send_json(response.model_dump())
                        ...
                        if last_vad_beg > -1 and last_vad_end > -1:
                            ...
                            result = (
                                ...
                                if sv and not hit
                                    ...
                                    audio_vad[beg:end], lang.strip(), cache_asr, True
                                )
                            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run the FastAPI app with a specified port."
    )
    parser.add_argument(
        "--port", type=int, default=27000, help="Port number to run the FastAPI app on."
    )
